...
# ── Confidential Secret ────────────────────────────────────────────────────
# Must be set via DJANGO_SECRET_KEY in production. If omitted in production
# the app refuses to start — this is a hard safety check, not a warning.
...
if _db_url:
    ...
    db_host = (
        os.environ.get("MARIADB_PRIVATE_HOST")
        or os.environ.get("MYSQL_HOST")
        or os.environ.get("MYSQLHOST")
        or ""
    )
    db_port_raw = (
        os.environ.get("MARIADB_PRIVATE_PORT")
