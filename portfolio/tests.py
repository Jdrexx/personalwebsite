...
class PortfolioPagesTests(TestCase):
    def test_home_page_loads_core_content(self):
        ...
        self.assertContains(
            response, "Technical Implementation & Customer Success"
        )
        self.assertContains(response, "Four-plus years owning software")
    ...
    def test_case_study_404_for_missing(self):
        response = self.client.get(
            reverse("portfolio:case_study", kwargs={"slug": "nonexistent"}), secure=True
        )
    ...
    def test_case_study_canonical_slug_serves_200(self):
        ...
        self.assertContains(
            response,
            'rel="canonical" href="https://jdreksler.com/case-study/archplanreview/"',
        )
...
class SeoInfrastructureTests(TestCase):
    ...
    def test_json_ld_is_server_rendered_and_valid(self):
        routes = [
            ...
            reverse("portfolio:projects"),
            reverse(
                "portfolio:service_detail", kwargs={"slug": "ai-workflow-automation"}
            ),
            reverse("portfolio:case_study", kwargs={"slug": "knowledgeassistant"}),
        ]
        for route in routes:
